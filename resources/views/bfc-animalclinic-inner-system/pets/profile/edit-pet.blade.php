    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <div class="relative bg-white rounded-lg shadow">
            <div class="p-4 md:p-5">
                <form action="{{ route('admin.pets.update', $pet->id) }}" method="POST" enctype="multipart/form-data" class="space-y-2">
                    @method('PUT')
                    @csrf
                    ...
                    <div>
                        <label for="edit-owner" class="block mb-2 text-sm font-medium text-gray-900">Owner</label>
                        <input type="text" id="edit-owner" value="{{ $pet->owner->Fname }} {{ $pet->owner->Mname ? $pet->owner->Mname . '. ' : '' }}{{ $pet->owner->Lname }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" autocomplete="off" required>
                        <input type="hidden" id="edit-owner_id" name="owner_id" value="{{ $pet->owner->id }}">
                        <div id="edit-owner-suggestions" class="text-gray-400 border-gray-300 rounded-md mt-2 max-h-40 overflow-y-auto"></div>
                    </div>
                    <div>
